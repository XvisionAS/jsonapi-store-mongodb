...
MongoStore._generateLookUp = function(resourceConfig) {
  const lookUps = []
  ...
  for (const attributeName in attributes) {
    ...
    if (attribute._settings && attribute._settings.__as) {
      ...
      //         },
      //       {
      //         $unwind:`$${attribute._settings.__as}`
      //       },{
      //         $match:{
      //           $expr:{
      ...
      console.log(resourceConfig.noForeignBindings)
      if (isValid && !resourceConfig.noForeignBindings[attributeName]) {
        lookUps.push({
          $lookup:{
            ...
            },
            {
              $match:{
                $expr:{
                  $eq:[`$${attribute._settings.__as}.olivier-fieldap-jsonapi GET /jsonapi/accounts 200 29.452 ms - -id`,'$$id']
                }
              }
            },{
              $project:{
...
MongoStore.prototype.initialise = async function(resourceConfig) {
  ...
  resourceConfig.noForeignBindings = resourceConfig.noForeignBindings || []
  this.lookUps = MongoStore._generateLookUp(resourceConfig)
  // create view
...
MongoStore.prototype.search = function(request, callback) {
  var self = this
  // var collection = self._db.collection(`view-${request.params.type}`)
...
MongoStore.prototype.find = function(request, callback) {
  // var collection = this._db.collection(`view-${request.params.type}`)
