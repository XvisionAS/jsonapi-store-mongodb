...
MongoStore.prototype._createIndexForRelationships = async function() {
  ...
  for (const name of relationshipAttributeNames) {
    try {
      await collection.createIndex({
        [`${name}.id`]:1
      })
    } catch(e) {
      console.log(`[eee] cannot create index '${name}.id' `, e)
    }
  }
...
MongoStore.prototype._createIndexForSearchParams = async function() {
  let searchParamsAsObject = this.resourceConfig.searchParams

  if (!searchParamsAsObject) {
    return
  }
  ...
  for (const key in searchParamsAsObject) {
    const searchParam = searchParamsAsObject[key]
    if (searchParamsAsObject && (searchParam._type !== 'any') && (searchParam._type !== 'object')) {
      if (relationshipAttributeNames.indexOf(key) === -1) {
  ...
  for (const name of searchParams) {
    try {
      ...
      console.log(`[eee] cannot create index '${name}'`, e)
    }
  }
}
...
MongoStore.prototype.initialise = async function(resourceConfig) {
  ...
  await this._createIndexForRelationships()
  await this._createIndexForSearchParams()
...
MongoStore.prototype.search = function(request, callback) {
  ...
  const collection = this._db.collection(fastSearch ? request.params.type : this._viewName)
  ...
  cursor.toArray(
    (toArrayError, documents) => {
      ...
      let index = 0
      if (!fastSearch && this.resourceConfig.foreignIncludes.length) {
        async.eachLimit(
          documents,
          10,
